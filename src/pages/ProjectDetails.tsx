...
import { api, ProjectDetailsResponse, Device, Category, useStore } from '../services/api';
import { 
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex border-b border-gray-200 gap-6 px-2">
        ...
        <TabBtn id="inquiries" label="لیست استعلام‌ها" icon={<ShoppingCart size={18} />} active={activeTab} set={setActiveTab} count={inquiries.length} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <Row label="کارشناس اقدام کننده" value={project.assigned_sales_manager_name || project.assigned_sales_manager_id} />
      ...
      {showInquiryModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6">
            <h3 className="font-bold text-lg mb-4">افزودن استعلام جدید</h3>
            {loadingModalData ? (
              <div className="py-8 text-center text-gray-500"><Loader2 className="animate-spin mx-auto mb-2"/>در حال دریافت لیست...</div>
            ) : (
              <form onSubmit={handleAddInquiry} className="space-y-4">
                {/* Category Select */}
                <div>
                  <label className="block text-sm mb-1 font-medium text-gray-700">1. انتخاب دسته‌بندی</label>
                  <select 
                    required
                    value={inquiryForm.category_id}
                    onChange={e => setInquiryForm({...inquiryForm, category_id: e.target.value, device_id: ''})}
                    className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-sky-500"
                  >
                ...
                {/* Device Select */}
                <div>
                  <label className="block text-sm mb-1 font-medium text-gray-700">2. انتخاب مدل دستگاه</label>
                  <select 
                    required
                    disabled={!inquiryForm.category_id}
                    value={inquiryForm.device_id}
                    onChange={e => setInquiryForm({...inquiryForm, device_id: e.target.value})}
                    className="w-full p-2 border rounded-lg outline-none focus:ring-2 focus:ring-sky-500 disabled:bg-gray-100 disabled:text-gray-400"
                  >
                    <option value="">
                      {!inquiryForm.category_id ? 'ابتدا دسته‌بندی را انتخاب کنید' : 'انتخاب کنید...'}
                    </option>
                ...
                {/* Quantity */}
                <div>
                  <label className="block text-sm mb-1 font-medium text-gray-700">3. تعداد</label>
                  <input 
                    type="number" 
                    min="1"
